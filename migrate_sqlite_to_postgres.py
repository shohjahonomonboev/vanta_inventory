...
pg_url = os.environ["DATABASE_URL"]  # set this before running

src = create_engine(sqlite_url, future=True)
 dst = create_engine(pg_url, future=True)

# Ensure destination schema exists (simple CREATE IF NOT EXISTS)
with dst.begin() as d:
    d.execute(text(
        """
...
with src.begin() as s, dst.begin() as d:
    inv = s.execute(text("SELECT id,name,buying_price,selling_price,quantity,profit,COALESCE(currency,'UZS') FROM inventory")).all()
    for r in inv:
        d.execute(text(
            """
    ...
    sales = s.execute(text("SELECT id,item_id,qty,sell_price,profit,sold_at FROM sales")).all()
    for r in sales:
        d.execute(text(
            """
